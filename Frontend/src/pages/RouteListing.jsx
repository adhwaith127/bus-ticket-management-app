...
// The boolean toggle flags that exist on a Route.
// Keeping them as a list makes it easy to render them all without
// repeating the same checkbox JSX 8 times.
const ROUTE_FLAGS = [
    ...
    <div className="p-6 md:p-10 min-h-screen bg-slate-50 animate-fade-in">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="7" className="px-6 py-8 text-center text-slate-500">No routes found.</td></tr>
              ) : routes.map(item => (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={getModalTitle()}>
        <div className="space-y-5">
          ...
          {/* Boolean flags — rendered from the ROUTE_FLAGS list */}
          <div>
              ...
              {ROUTE_FLAGS.map(flag => (
                <label key={flag.name} className={`flex items-center gap-2 text-sm cursor-pointer p-2 rounded-lg border ${formData[flag.name] ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200'}`}>
                  <input
                    ...
                    disabled={isReadOnly}
                    className="sr-only"   // visually hidden — the whole label acts as the toggle
                  />
                  {flag.label}
                </label>
              ))}

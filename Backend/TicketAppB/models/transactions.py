...
class TripOpenData(models.Model):
    # ── Identity ─────────────────────────────────────────────────────────────
    unique_code = models.CharField(max_length=30, db_index=True)
    # should be foreign key once the device-company mapping is complete
    palmtec_id  = models.CharField(max_length=20, db_index=True)
    # device_id = models.ForeignKey('DeviceRegistry', on_delete=models.SET_NULL, null=True, blank=True, related_name='trip_opens')

    # ── Route / Trip ─────────────────────────────────────────────────────────
    # route_code = models.CharField(max_length=50, null=True, blank=True)
    route_id = models.ForeignKey('Route', on_delete=models.SET_NULL, null=True, blank=True, related_name='trip_opens')
    ...
    # ── Vehicle / Crew ────────────────────────────────────────────────────────
    bus_no = models.CharField(max_length=30, null=True, blank=True)
    # bus_id = models.ForeignKey('Vehicle', on_delete=models.SET_NULL, null=True, blank=True, related_name='trip_opens')

    driver = models.CharField(max_length=50, null=True, blank=True)
    # driver_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='driven_trip_opens')

    conductor = models.CharField(max_length=50, null=True, blank=True)
    # conductor_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='conducted_trip_opens')
    ...
    # ── Device telemetry ──────────────────────────────────────────────────────
    battery_percentage = models.IntegerField(null=True, blank=True)
    total_record       = models.IntegerField(null=True, blank=True)

    # ── Relations ─────────────────────────────────────────────────────────────
    company_code = models.ForeignKey(
        Company, on_delete=models.PROTECT,
        related_name='trip_opens', db_index=True, null=True, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['palmtec_id', 'company_code', 'trip_no', 'start_date', 'start_time'],
    ...
    def __str__(self):
        return f"TripOpen {self.trip_no} - {self.palmtec_id} ({self.start_datetime})"


class TripCloseData(models.Model):
    # ── Identity ─────────────────────────────────────────────────────────────
    unique_code = models.CharField(max_length=30, db_index=True, null=True, blank=True)
    # should be foreign key once the device-company mapping is complete
    palmtec_id  = models.CharField(max_length=50, db_index=True)

    # ── Route / Trip ─────────────────────────────────────────────────────────
    # route_code = models.CharField(max_length=50, db_index=True)
    route_id = models.ForeignKey('Route', on_delete=models.SET_NULL, null=True, blank=True, related_name='trip_closes')

    schedule = models.IntegerField()
    # schedule_id = models.ForeignKey('Schedule', on_delete=models.SET_NULL, null=True, blank=True, related_name='trip_closes')
    ...
    # ── Vehicle / Crew ────────────────────────────────────────────────────────
    driver = models.CharField(max_length=50, null=True, blank=True)
    # driver_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='driven_trip_closes')

    conductor = models.CharField(max_length=50, null=True, blank=True)
    # conductor_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='conducted_trip_closes')
    ...
    # ── Distance ──────────────────────────────────────────────────────────────
    total_km         = models.DecimalField(max_digits=8, decimal_places=2, default=Decimal('0.00'), null=True, blank=True)
    ...
    # ── Passenger counts ─────────────────────────────────────────────────────
    full_count     = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    half_count     = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    st1_count      = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    luggage_count  = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    physical_count = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    pass_count     = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    ladies_count   = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    senior_count   = models.IntegerField(default=0, validators=[MinValueValidator(0)])

    total_tickets      = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    total_cash_tickets = models.IntegerField(default=0, validators=[MinValueValidator(0)])

    # ── Collection amounts ────────────────────────────────────────────────────
    full_collection     = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    half_collection     = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    st_collection       = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    luggage_collection  = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    physical_collection = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    ladies_collection   = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    senior_collection   = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    adjust_collection   = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    expense_amount      = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), validators=[MinValueValidator(Decimal('0.00'))])
    total_collection    = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), validators=[MinValueValidator(Decimal('0.00'))])
    total_cash_amount   = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), validators=[MinValueValidator(Decimal('0.00'))])

    # ── UPI breakdown ─────────────────────────────────────────────────────────
    upi_ticket_count  = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    upi_ticket_amount = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), validators=[MinValueValidator(Decimal('0.00'))])

    # ── Relations ─────────────────────────────────────────────────────────────
    company_code = models.ForeignKey(
        Company, on_delete=models.PROTECT,
        related_name='trips', db_index=True, null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes  = [
            models.Index(fields=['palmtec_id', 'start_datetime']),
            models.Index(fields=['route_id', 'start_datetime']),
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['palmtec_id', 'company_code', 'schedule', 'trip_no', 'start_datetime'],
                name='uniq_device_trip_close'
            )
        ]
        ordering = ['-start_datetime']

    def __str__(self):
        return f"Trip {self.trip_no} - {self.palmtec_id} ({self.start_datetime})"

    def get_total_passengers(self):
        return (
            self.full_count + self.half_count + self.st1_count +
            self.physical_count + self.pass_count +
            self.ladies_count + self.senior_count
        )
    ...
    def calculate_total_tickets(self):
        return (
            self.full_count + self.half_count + self.st1_count +
            self.luggage_count + self.physical_count + self.pass_count +
            self.ladies_count + self.senior_count
        )
...
class ScheduleOpenData(models.Model):
    # ── Identity ─────────────────────────────────────────────────────────────
    unique_code = models.CharField(max_length=30, db_index=True)
    # should be foreign key once the device-company mapping is complete
    palmtec_id  = models.CharField(max_length=20, db_index=True)
    ...
    # ── Vehicle / Crew ────────────────────────────────────────────────────────
    bus_no = models.CharField(max_length=30, null=True, blank=True)
    # bus_id = models.ForeignKey('Vehicle', on_delete=models.SET_NULL, null=True, blank=True, related_name='schedule_opens')

    driver = models.CharField(max_length=50, null=True, blank=True)
    # driver_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='driven_schedule_opens')

    conductor = models.CharField(max_length=50, null=True, blank=True)
    # conductor_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='conducted_schedule_opens')
    ...
    company_code = models.ForeignKey(
        Company, on_delete=models.PROTECT,
        related_name='schedule_opens', db_index=True, null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes  = [
            ...
            models.Index(fields=['start_date']),
            models.Index(fields=['unique_code']),
            models.Index(fields=['schedule_no']),
        ]
        constraints = [
            models.UniqueConstraint(
    ...
    def __str__(self):
        return f"ScheduleOpen {self.schedule_no} - {self.palmtec_id} ({self.start_datetime})"


class ScheduleCloseData(models.Model):
    # ── Identity ─────────────────────────────────────────────────────────────
    unique_code = models.CharField(max_length=30, db_index=True, null=True, blank=True)
    palmtec_id  = models.CharField(max_length=50, db_index=True)

    # ── Route / Schedule ──────────────────────────────────────────────────────
    route_id    = models.ForeignKey('Route', on_delete=models.SET_NULL, null=True, blank=True, related_name='schedule_closes')
    schedule_no = models.IntegerField(db_index=True)

    # ── Vehicle / Crew ────────────────────────────────────────────────────────
    bus_no = models.CharField(max_length=30, null=True, blank=True)
    # bus_id = models.ForeignKey('Vehicle', on_delete=models.SET_NULL, null=True, blank=True, related_name='schedule_closes')

    driver = models.CharField(max_length=50, null=True, blank=True)
    # driver_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='driven_schedule_closes')

    conductor = models.CharField(max_length=50, null=True, blank=True)
    # conductor_id = models.ForeignKey('Employee', on_delete=models.SET_NULL, null=True, blank=True, related_name='conducted_schedule_closes')
    ...
    company_code = models.ForeignKey(
        Company, on_delete=models.PROTECT,
        related_name='schedule_closes', db_index=True, null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes  = [
            models.Index(fields=['palmtec_id', 'end_datetime']),
            models.Index(fields=['route_id', 'end_datetime']),
            models.Index(fields=['company_code', 'end_date']),
            models.Index(fields=['company_code']),
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['palmtec_id', 'company_code', 'schedule_no', 'end_datetime'],
    ...
    def __str__(self):
        return f"ScheduleClose {self.schedule_no} - {self.palmtec_id} ({self.end_datetime})"

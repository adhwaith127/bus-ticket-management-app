...
class DeviceSettings(models.Model):
    """
    Per-device settings for registered ETM devices.
    Each ETMDevice gets its own configurable settings profile.
    No FTP / network / smart-card fields — those are omitted by design.
    """
    ...
    # ── ST (Student) fare settings ───────────────────────────────────────────
    # When st_fare_edit is True → hide the three ST fields below
    st_fare_edit = models.BooleanField(default=False, help_text="When ON, ST fare fields are hidden")
    st_max_amt   = models.CharField(max_length=5, default='0', blank=True, help_text="ST max amount (max 5 chars incl. decimal)")
    st_ratio     = models.CharField(max_length=2, default='0', blank=True, help_text="ST ratio max 2 digits")
    st_min_amt   = models.CharField(max_length=6, default='0', blank=True, help_text="ST min amount (max 6 chars incl. decimal)")

    # ── ST Roundoff ──────────────────────────────────────────────────────────
    # When st_roundoff_enable is True → show st_roundoff_amt
    st_roundoff_enable = models.BooleanField(default=False)
    st_roundoff_amt    = models.CharField(max_length=3, default='0', blank=True, help_text="Roundoff in paise (max 3 chars incl. decimal)")
    ...
    created_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name='device_settings_created',
    )
    updated_at = models.DateTimeField(auto_now=True)
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name='device_settings_updated',
    )
    ...
    def __str__(self):
        return f"Settings for device {self.device.serial_number}"


class SettingsProfile(models.Model):
    """
    Named settings template that can be applied to one or more ETM devices.
    Fields mirror DeviceSettings so a profile can be copied to a device directly.
    """

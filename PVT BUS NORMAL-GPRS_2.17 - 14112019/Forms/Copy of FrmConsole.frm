...
FrmConsole.Enabled = False
        ...
        TSQL = ""
        TSQL = "select Sum([Full]) as ful,Sum(Half) as Haf,Sum(LuggAmont) AS lugamt,Sum(Phy) as ph,Sum(st) as stu,Sum(Amount) as amt,FromStage,ToStage,PalmId,Schdule,TripNo from TKTS where DATEVALUE(Date) BETWEEN DATEVALUE('" & DTfrom.Value & " ') AND DATEVALUE('" & DTTo.Value & "')"
...
'                ExlObj.ActiveSheet.Cells(exclrow, 2).Value = getvalueQuery("SELECT TOP 1 StageName From Stage WHERE route='" & routecode & "' and  StageName  not in (SELECT TOP " & Myres("ToStage") & " StageName  from STAGE where  route='" & routecode & "' ORDER BY id)ORDER BY id")
                ...
                ExlObj.ActiveSheet.Cells(exclrow, 7).Value = Myres("LugAmt")
...
'                    passcnt = passcnt + 1
'                End If
                ExlObj.ActiveSheet.Cells(exclrow, 8).Value = Myres("Amt")
...
'                ExlObj.ActiveSheet.Cells(exclrow, 12).Value = Myres("Date")
                ExlObj.ActiveSheet.Cells(exclrow, 9).Value = Myres("PalmId")
                ExlObj.ActiveSheet.Cells(exclrow, 10).Value = Myres("Schdule")
                ExlObj.ActiveSheet.Cells(exclrow, 11).Value = Myres("TripNo")
                Myres.MoveNext
                exclrow = exclrow + 1
            ...
            ExlObj.ActiveSheet.Cells(exclrow, 6).Font.Bold = True
            ExlObj.ActiveSheet.Cells(exclrow, 6).Font.Color = vbBlack
            ExlObj.ActiveSheet.Cells(exclrow, 7).Value = luggcnt
            ExlObj.ActiveSheet.Cells(exclrow, 7).Font.Bold = True
            ExlObj.ActiveSheet.Cells(exclrow, 7).Font.Color = vbBlack
            ExlObj.ActiveSheet.Cells(exclrow, 8).Value = Format(TotAmt, "0.00")
            ExlObj.ActiveSheet.Cells(exclrow, 8).Font.Bold = True
            ExlObj.ActiveSheet.Cells(exclrow, 8).Font.Color = vbBlack
            exclrow = exclrow + 1
             exclrow = exclrow + 1
            ...
            ExlObj.ActiveSheet.Cells(exclrow, 1).Font.Bold = True
            ExlObj.ActiveSheet.Cells(exclrow, 1).Font.Color = vbBlack
            ExlObj.ActiveSheet.Cells(exclrow, 4).Value = TotAmt
            ExlObj.ActiveSheet.Cells(exclrow, 4).Font.Bold = True
            ExlObj.ActiveSheet.Cells(exclrow, 4).Font.Color = vbBlack
...
Dim pass As Long
        TSQL = ""
        TSQL = "select Sum([Full]) as ful,Sum(Half) as Haf,Sum(LuggAmont) AS lugamt,Sum(Phy) as ph,Sum(st) as stu,Sum(Amount) as amt,FromStage-1 as FromStage ,ToStage-1 as ToStage, PalmId,Schdule,TripNo from TKTS where DATEVALUE(Date) BETWEEN DATEVALUE('" & DTfrom.Value & " ') AND DATEVALUE('" & DTTo.Value & "')"
        If cmbPalmID.Text <> "All" Then
            TSQL = TSQL & " and PalmId='" & cmbPalmID.Text & "'"
        End If
        If cmbSchedule.Text <> "All" Then

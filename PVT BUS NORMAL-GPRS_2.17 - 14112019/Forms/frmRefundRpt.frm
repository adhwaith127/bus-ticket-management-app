...
Begin VB.Form frmRefundRpt 
   ...
   End
End
Attribute VB_Name = "frmRefundRpt"
...
On Error Resume Next
    ...
    If cmbPalmID <> "" Then
        If cmbPalmID.Text = "All" Then
            sql = "select distinct SCHEDULE from TKTS where DATEVALUE(date) BETWEEN DATEVALUE('" & DTStart.Value & " ') AND DATEVALUE('" & DTEnd.Value & "') "
        Else
            sql = "select distinct SCHEDULE from TKTS where PalmID='" & cmbPalmID.Text & "' AND DATEVALUE(date) BETWEEN DATEVALUE('" & DTStart.Value & " ') AND DATEVALUE('" & DTEnd.Value & "') "
        End If
...
On Error Resume Next
    ...
    If cmbPalmID <> "" Then
        If cmbPalmID.Text = "All" Then
            sql = "select distinct SCHDULE from TKTS where  DATEVALUE(date) BETWEEN DATEVALUE('" & DTStart.Value & " ') AND DATEVALUE('" & DTEnd.Value & "') "
        Else
            sql = "select distinct SCHDULE from TKTS where PalmID='" & cmbPalmID.Text & "' AND DATEVALUE(date) BETWEEN DATEVALUE('" & DTStart.Value & " ') AND DATEVALUE('" & DTEnd.Value & "') "
        End If
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
        If cmbPalmID.Text <> "All" Then
        If RES.RecordCount > 0 Then
...
Private Sub cmbShedule_Change()
    ...
    If cmbPalmID <> "" Then
        If cmbShedule <> "" Then
        sql = "SELECT DISTINCT TRIPNO  FROM TKTS WHERE PALMID='" & cmbPalmID.Text & "' and SCHEDULE='" & cmbShedule.Text & "'"
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
...
cmbtripno.Enabled = True
    ...
    If cmbPalmID <> "" Then
        If cmbShedule <> "" And cmbShedule <> "All" Then
         cmbtripno.Clear
        sql = "SELECT DISTINCT TRIPNO  FROM TKTS WHERE PALMID='" & cmbPalmID.Text & "' and SCHDULE=" & cmbShedule.Text & ""
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
        If RES.EOF <> True Then
            RES.MoveFirst
             cmbtripno.AddItem "All"
...
On Error Resume Next
 Dim mystr As String
    ...
    End If
     sql = ""
    sql = "select TicketNo,FromStage,tostage,Amount as TKTAMT,REFUNDAMT,(val(Amount)-val(REFUNDAMT)) as BALANCE ,DAte,palmid,schdule as SCHEDULE,tripno as TRIP from TKTS where REFUNDSTS=1 and DATEVALUE(Date) BETWEEN DATEVALUE('" & DTStart.Value & " ') AND DATEVALUE('" & DTEnd.Value & "')"
    If cmbPalmID.Text <> "All" Then
            sql = sql & " and PalmId='" & cmbPalmID.Text & "'"
    End If
    If cmbShedule.Text <> "All" Then
        sql = sql & " and Schdule=" & cmbShedule.Text & ""
    End If
    If cmbtripno.Text <> "All" Then
        sql = sql & " and TripNo=" & cmbtripno.Text & ""
    End If
    sql = sql & " ORDER BY Date,PalmId,Schdule,TripNo,TicketNo"
    ...
    Set RefundRpt.Icon = frmMainform.Icon
    Set RefundRpt.DataSource = rs
    RefundRpt.Show vbModal
    Else
...
On Error Resume Next
    sql = "select DISTINCT PALMID from TKTS where DATEVALUE(date) BETWEEN DATEVALUE('" & DTStart.Value & " ')AND DATEVALUE('" & DTEnd.Value & "') "
    Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
    cmbPalmID.Clear
    cmbShedule.Clear
    cmbtripno.Clear
    If RES.RecordCount > 0 Then
        ...
        RES.Close
         cmbShedule.Clear
        sql = "select DISTINCT SCHEDULE from TKTS where PalmId='" & cmbPalmID.Text & "' and DATEVALUE(StartDate) BETWEEN DATEVALUE('" & DTStart.Value & " ')AND DATEVALUE('" & DTEnd.Value & "') "
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
        RES.MoveFirst
        Do While Not RES.EOF
        ...
        RES.Close
        'cmbtripno.Clear
        sql = "SELECT DISTINCT TRIPNO  FROM TKTS WHERE PALMID='" & cmbPalmID.Text & "' and DATEVALUE(StartDate) BETWEEN DATEVALUE('" & DTStart.Value & " ')AND DATEVALUE('" & DTEnd.Value & "') "
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
...
On Error Resume Next
    ...
    CONNECTDB
    sql = "SELECT DISTINCT PALMID FROM TKTS"
    Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
   If 1 Then
        DTEnd.Visible = True
      ...
      '  RES.Close
        If cmbPalmID.Text = "All" Then
            sql = "SELECT DISTINCT SCHDULE  FROM TKTS"
        Else
            sql = "SELECT DISTINCT SCHDULE  FROM TKTS WHERE PALMID='" & cmbPalmID.Text & "'"
        End If
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
        If RES.RecordCount > 0 Then RES.MoveFirst
        cmbShedule.Clear
        cmbShedule.AddItem "All"
        Do While Not RES.EOF
            cmbShedule.AddItem RES!SCHDULE
            RES.MoveNext
        Loop
        If cmbShedule.ListCount > 0 Then
            cmbShedule.Text = cmbShedule.List(0)
        ...
        sql = "SELECT DISTINCT TRIPNO  FROM TKTS WHERE PALMID='" & cmbPalmID.Text & "'"
        Set RES = CNN.OpenRecordset(sql, dbOpenDynaset)
        If RES.RecordCount > 0 Then RES.MoveFirst
        cmbtripno.Clear
        cmbtripno.AddItem "All"
